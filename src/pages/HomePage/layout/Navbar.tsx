  ...
  return (
    <motion.nav
      ...
      className={`fixed w-full z-50 transition-all duration-300 ${
        isScrolled ? "py-2 bg-black/30 backdrop-blur-xl border-b border-cyan-900/30 shadow-lg shadow-cyan-900/10" : "py-4 bg-transparent"
      }`}>
              ...
              if (item.isLink) {
                return (
                  <Link to={`${item.link}`}>
                    ...
                    ${activeLink === item.name.toLowerCase() ? "text-white" : "text-gray-300 hover:text-white hover:bg-white/5"}`}
              ...
              <AnimatePresence>
                {isDropdownOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    transition={{ duration: 0.2 }}
                    className='absolute top-full left-0 mt-2 w-48 bg-cyan-950 backdrop-blur-xl rounded-lg shadow-lg border border-cyan-900/30'>
              ...
              <div className='flex gap-3'>
                ...
                <Link to='/signup'>
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() => {}}
                    className='bg-gradient-to-r from-cyan-500 to-sky-600 text-white px-6 py-2.5 rounded-xl font-medium  transition-all duration-300 flex items-center gap-2 text-sm'>
                    <span>SignUp</span>
                  </motion.button>
                </Link>
              </div>
        ...
        {isMobileMenuOpen && (
          <motion.div
            ...
            <motion.div
              ...
              className='px-4 pt-2 pb-6 space-y-1 bg-gradient-to-b from-black/80 to-cyan-950/80 backdrop-blur-xl border-b border-cyan-900/30'>
              {navLinks.map((item, index) => (
                <motion.a
                  ...
                  className={`block px-4 py-3 rounded-lg text-sm font-medium transition-colors duration-200 ${
                    activeLink === item.name.toLowerCase()
                      ? "bg-gradient-to-r from-cyan-500/20 to-sky-500/20 text-white"
